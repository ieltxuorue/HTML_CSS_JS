...
function obtener_ganador2(){
	...
	//RESETEAR LAS CELDAS Y CAMBIAR POR SU NOMBRE 
	var all_cells = document.getElementsByTagName('td');
	for (var i = 0; i<all_cells.length; i++) {
		all_cells[i].className = 'negro';
		all_cells[i].innerHTML = names[i];
	}
	
	//OBTENER EL NUMERO ALEATORIO
	var lbl_num_afortunado = document.getElementById('num_afortunado');
	var num_aleatorio = Math.floor(Math.random() * 15) +1;
	
	//SELECCIONAR LA CELDA Y CAMBIARLE LA CLASE
	var random_cell = 'c' + num_aleatorio;
	document.getElementById(random_cell).className = 'verde';
	
	//MOSTRAR EL NUMERO
	lbl_num_afortunado.innerHTML = num_aleatorio;
